...
Agent-based model with Person and Place agents using Repast4Py.
Implements movement, behavior, environmental interaction, and place-based updates
in a spatially explicit, parallelized context.
"""
...
class Person(core.Agent):
    ...
    def __init__(self, agent_id, rank, place_id, behavior_id, social_links=None, environment=None):
        ...
        self.environment = np.zeros(2, dtype=np.float32) if environment is None else np.array(environment, dtype=np.float32)
        self.at_remote_place = False

    def move(self, dest_place):
        self.place_id = dest_place.id
...
class SocialModel:
    def __init__(self, comm: MPI.Intracomm, params: dict):
        required_keys = ["num_persons", "num_places", "grid_resolution"]
        for key in required_keys:
            if key not in params:
                raise ValueError(f"Missing required parameter: {key}")

        num_persons = int(params.get("num_persons", 1000))
        num_places = int(params.get("num_places", 100))
        grid_resolution = int(params.get("grid_resolution", 1000))
    ...
    def _init_places(self, count):
        for i in range(count):
            ...
            self.place_index[place.id] = place
    ...
    def _init_persons(self, count):
        ...
        for i in range(count):
            ...
            self.person_index[person.id] = person
    ...
    def _get_env_slice(self):
        ...
        idx = min(idx, len(self.time_dim) - 1)
        return self.env_data[self.env_var].isel(time=idx).values

    def step(self):
        print(f"[Rank {rank}] Tick {self.current_tick} at {self.current_datetime}")
        self.current_tick += 1
        ...
        for person in self.persons:
            ...
            dest_place = self.place_index.get(dest_id)
            if dest_place:
                person.move(dest_place)
                if not person.at_remote_place:
                    self.space.move(person, ContinuousPoint(dest_place.x, dest_place.y))
                    gx = int((dest_place.x - self.min_x) / self.grid_resolution)
                    gy = int((dest_place.y - self.min_y) / self.grid_resolution)
    ...
    def _place_updates(self, hour, day):
        print(f"[Rank {rank}] Performing place updates for hour={hour}, day={day}")
...
def run_model(params: dict):
    model = SocialModel(MPI.COMM_WORLD, params)
    for _ in range(10):
